...
def draw(data):
    ...
    for i in range(data.shape[0]-2):
        ...
        if roc(v1, v2) > abs(d) and inangle(v1, v2) < 0.95*math.pi:
            ...
            for j in range(0, data.shape[0]):
                if np.linalg.norm(new-data[j, :]) < abs(d)*0.999:
                    k = 1
                    break
#            if np.linalg.norm(new-data[i+1, :]) > abs(d)*5 or (temp.shape[0] > 1 and np.linalg.norm(new-temp[temp.shape[0]-1]) < abs(d)*0.5):
#                k = 1
            if k == 0:
    ...
    plt.plot(data[:, 0], data[:, 1], '-o', color='r', markersize=1)
    return(temp)
